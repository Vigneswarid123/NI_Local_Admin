...
import { usersModel, usersPwdModel, dealerusersPwdModel } from '../../_models/users';
...
export class ApiService {
  ...
  getToken() {
    if (localStorage.getItem('thrott_token') !== null) {
      ...
      return sessionStorage.getItem('thrott_token');
    }
  }
  ...
  updatePwd(model: usersPwdModel) {
    return this.http.post(`${environment.apiUrl}users/change`, model);
  }
  ...
  getRolesData(data) {
    const API_URL1 = `${environment.apiUrl}roles/get`;
  ...
  updateDealerUserPwd(model: dealerusersPwdModel) {
